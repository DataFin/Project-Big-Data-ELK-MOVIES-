...
| US-05 | En tant qu'analyste, je veux 12 requêtes DSL commentées pour explorer les données films | 5, 5, 8, 5 | 5 | Les requêtes couvrent des cas métier réels | Sandrine |
| US-06 | En tant que manager, je veux un dashboard Kibana pour visualiser les tendances du cinéma | 5, 8, 8, 8 | 8 | Kibana est opérationnel et movies_clean est indexé | Destiné |
| US-07 | En tant que développeur, je veux une documentation complète pour que le projet soit reproductible | 3, 3, 5, 3 | 3 | La documentation couvre tous les aspects techniques | Audrey |
...
|---|---|---|
| **Sandrine** (lead) | F2 Ingestion brute + F5 Requêtes DSL + normalisation F3 | 3 + 5 + 3 = 11 |
| **Audrey** | F1 Bootstrap stack + F7 Documentation + anomalies F3 | 3 + 3 + 2 = 8 |
| **Destiné** | F3 Nettoyage (types + date) + F6 Dashboard Kibana | 3 + 8 = 11 |
...
F4 Mapping & qualité (Alexandre)
        ↓
F5 Requêtes analytiques (Sandrine)
        ↓
F6 Dashboard Kibana (Destiné)
        ↓
F7 Documentation (Audrey)
        ↓
